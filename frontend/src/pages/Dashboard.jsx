...
const Dashboard = () => {
  ...
  // Fixed fetchLeads function - removed problematic dependencies
  const fetchLeads = useCallback(async (page = 1, currentFilters = {}, searchQuery = '') => {
    setLoading(true);
    try {
      const params = {
        page,
        limit: 20, // Use fixed limit instead of pagination.limit
        ...currentFilters
      };

      // Add search term to filters if present
      if (searchQuery.trim()) {
        params.company_contains = searchQuery.trim();
      }

      const response = await leadsAPI.getLeads(params);
      const { data, page: responsePage, limit, total, totalPages } = response.data;
  ...
  // Initial load effect
  useEffect(() => {
    fetchLeads(1, filters, searchTerm);
  }, []); // Only run on mount
  ...
  const handleFilterChange = (filterKey, value) => {
    ...
    fetchLeads(1, newFilters, searchTerm);
  };
  ...
  const handleDeleteLead = async (id) => {
    ...
    try {
      ...
      // Refresh current page with current filters and search
      fetchLeads(pagination.page, filters, searchTerm);
    } catch (error) {
  ...
  const handlePageChange = (newPage) => {
    fetchLeads(newPage, filters, searchTerm);
  };
